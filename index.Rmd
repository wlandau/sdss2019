...
```{r c1, include = FALSE}
options(warnPartialMatchArgs = FALSE, drake_make_menu = FALSE)
packages <- c(
  "drake",
  "keras",
  "recipes",
...
## Example workflow: deep learning customer churn
...
<li>See also `drake_ggraph()` and `sankey_drake_graph()`</li>
</ul>
...
```{r, plan2}
data_source_index <- seq_len(25)
plan <- drake_plan(
  data = target(
    download_dataset(source),
    transform = map(source = !!data_source_index)
  ),
  history = target(
    fit_my_model(data, batch_size = bz, rate = lr, dropout = dr),
    transform = cross(
      data,
      bz = c(8, 16, 32, 64),
      lr = c(1e-5, 1e-4, 1e-3),
      dr = c(0.2, 0.3, 0.4, 0.5)
    )
  ),
  best = target(
...
- [Today's code](https://github.com/wlandau/drake-examples/tree/master/tensorflow): `drake_example("tensorflow")`
